...
// ログを記録する共通関数
export async function runTestWithLogs(fixtureName: string, checkerOption?: string) {
  ...
  const tscResult = await $`npx ${checker} --noEmit`.cwd(afterDir).nothrow().quiet();
  ...
  // CLIを実行してエラーを抑制
  const args = checkerOption ? ['--project', afterDir, `--checker=${checkerOption}`] : ['--project', afterDir];
  await $`bun run src/index.ts ${args}`;
...
// プロジェクト内の全TypeScriptファイルを取得
export function getAllTsFiles(dir: string, baseDir?: string): Record<string, string> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      Object.assign(files, getAllTsFiles(fullPath, base));
    } else if (entry.name.endsWith('.ts') || entry.name.endsWith('.tsx')) {
      const relativePath = require('path').relative(base, fullPath);
